...
def main():
    ...
    data_upper = os.path.join('tmp', 'librispeech-lm-norm.txt.gz')
...
#    # Build pruned LM.
    ...
    subprocess.check_call([
      '/home/beomgon/kenlm/build/bin/lmplz', '--order', '5',
               '--temp_prefix', 'tmp',
               '--memory', '50%',
               '--text', data_lower,
               '--arpa', lm_path,
    ...
    subprocess.run(['/home/beomgon/kenlm/build/bin/filter', 'single', 'model:{}'.format(lm_path), filtered_path], input=vocab_str.encode('utf-8'), check=True)

    # Quantize and produce trie binary.
    print('Building lm.binary...')
    subprocess.check_call([
      '/home/beomgon/kenlm/build/bin/build_binary', '-a', '255',
                      '-q', '8',
                      'trie',
                      lm_path,
                      'lm.binary'
