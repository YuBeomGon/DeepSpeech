...
def create_flags():
    ...
    f.DEFINE_boolean('isbatchnorm', True, 'flag for phase(training or test) for batch normalization')
    ...
    f.DEFINE_float('learning_rate', 0.000002, 'learning rate of Adam optimizer')

    # Batch sizes

    f.DEFINE_integer('train_batch_size', 1, 'number of elements in a training batch')
    f.DEFINE_integer('dev_batch_size', 32, 'number of elements in a validation batch')
    f.DEFINE_integer('test_batch_size', 32, 'number of elements in a test batch')
    ...
    f.DEFINE_integer('report_count2', 630, 'number of phrases with lowest WER(best matching) to print out during a WER report')

    homedir = os.path.expanduser('~')
    f.DEFINE_string('result_dir', homedir + '/DeepSpeech/data/Zeroth/result1', 'target directory for TensorBoard summaries - defaults to directory "deepspeech/summaries" within user\'s data home specified by the XDG Base Directory Specification')

    f.DEFINE_string('summary_dir', 'data/KO/summary', 'target directory for TensorBoard summaries - defaults to directory "deepspeech/summaries" within user\'s data home specified by the XDG Base Directory Specification')
